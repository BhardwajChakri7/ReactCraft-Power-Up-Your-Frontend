...
class AppStore extends Component {
  state = {onActiveTab: tabsList[0].tabId, searchInput: ''}

  currentTab = tabId => {
    this.setState({onActiveTab: tabId})
  }
  ...
  render() {
    const {onActiveTab, searchInput} = this.state
    const filteredItems = appsList.filter(
      eachApp => onActiveTab === eachApp.category,
    )
    return (
        ...
        <ul className="nav-list">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              isActive={onActiveTab === eachItem.tabId}
            />
          ))}
        </ul>
