...
class App extends Component {
  ...
  tick = () => {
    this.setState(prev => {
      if (prev.timeRemaining > 1) {
        ...
        clearInterval(this.timerId)
        return {timeRemaining: 0, isGameOver: true}
      }
    })
  }
  ...
  checkIsMatched = id => {
    ...
    if (randomImg.id === id) {
      ...
      this.setState(prev => ({count: prev.count + 1, randomImg: newRandomImg}))
    } else {
      clearInterval(this.timerId)
  ...
  render() {
    const {activeTabId, randomImg, count, timeRemaining, isGameOver} =
      this.state
        ...
        <header>
          <nav className="main-nav">
            <ul className="nav-bar">
              ...
              <li className="sub-part1">
                <p className="nav-para">
                  Score: <span className="score-element">{count}</span>
                </p>
                <li className="sub-part2">
                  <img
                    src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                    alt="timer"
                    className="timer-img"
                  />
                  <p className="main-timer">{timeRemaining} sec</p>
                </li>
              </li>
            </ul>
          </nav>
        ...
        {isGameOver ? (
          ...
          <div className="main-bg-container">
            <div className="sub-img-container">
              <img src={randomImg.imageUrl} alt="match" className="main-img" />
            </div>
            ...
            <ul className="thumbnail-container">
              {filteredProjects.map(image => (
                <ImageDisplay
                  key={image.id}
                  eachImage={image}
                  checkIsMatched={this.checkIsMatched}
                />
              ))}
            </ul>
          </div>
